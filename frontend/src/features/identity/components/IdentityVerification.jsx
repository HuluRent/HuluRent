...
export function IdentityVerification() {
  const { data: statusData, isLoading, isError, error } = useIdentityVerificationStatus();
  ...
  if (isError) {
    return (
      <div className="p-6 bg-red-50 border border-red-100 rounded-2xl shadow-sm text-center text-red-600">
        ...
        <p>Failed to load identity verification status: {error?.message}</p>
      </div>
    );
  }
  ...
  const handleInitiate = (e) => {
    ...
    initiateMut.mutate(idNumber, {
      onSuccess: () => {
        setSuccessMessage('Verification initiated. Please check your email for the OTP.');
      },
      onError: (err) => {
        setErrorMessage(err.response?.data?.error || err.message || 'Failed to initiate verification');
      }
    });
  };
        ...
        <div>
          <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl text-amber-800 mb-4">
            ...
            <div>
              ...
              <p className="text-sm opacity-90">Please enter the OTP sent to your email.</p>
            </div>
          </div>
          ...
          <form onSubmit={handleVerify} className="flex flex-col gap-3 max-w-sm">
            ...
            >
              {verifyMut.isPending ? <LoadingSpinner size="sm" /> : 'Verify Identity'}
            </button>
          </form>
        </div>
      )}
      ...
      {status === 'UNVERIFIED' && (
        <div>
          <p className="text-text-muted mb-4 text-sm leading-relaxed">
            Verifying your identity helps build trust in the HuluRent community. Please enter your valid ID number to begin the verification process.
          </p>

          <form onSubmit={handleInitiate} className="flex flex-col gap-3 max-w-sm">
            <input
              type="password"
              placeholder="Enter ID Number"
              value={idNumber}
            ...
            <button
              ...
              className="hr-btn-primary flex justify-center items-center gap-2 disabled:opacity-70"
            >
              {initiateMut.isPending ? <LoadingSpinner size="sm" /> : 'Start Verification'}
            </button>
          </form>
        </div>
      )}
