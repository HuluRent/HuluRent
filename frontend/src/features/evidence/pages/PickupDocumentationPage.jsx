...
export default function PickupDocumentationPage() {
  ...
  const pickupEvidence = allEvidence.filter((e) => e.type === 'PICKUP');

  if (isLoading) return <LoadingSpinner label="Loading documentation…" />;

  const hasEvidence = pickupEvidence.length > 0;
  const latestEvidence = hasEvidence ? pickupEvidence[pickupEvidence.length - 1] : null;
  ...
  return (
    <div className="max-w-3xl mx-auto">
          ...
          {latestEvidence.photoUrls?.length > 0 && (
            <div className="flex flex-wrap gap-3 mb-4">
              {latestEvidence.photoUrls.map((url, i) => (
                <img key={i} src={url} alt={`Pickup photo ${i + 1}`} className="w-28 h-28 rounded-xl object-cover border border-outline-variant" />
              ))}
            </div>
          )}
      ...
      {!hasEvidence && (
        <ConditionForm bookingId={bookingId} evidenceType="PICKUP" />
      )}
    </div>
  );
}
